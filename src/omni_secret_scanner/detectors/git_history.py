...
def scan_history(
    ...
    findings: dict = {
        "secrets": [], "pii": [], "entropy": [], "commits": [], "injections": [],
    }
    ...
    for commit_hash, message in scan_commit_messages(all_branches, repo_cwd=repo_cwd):
        ...
        for word in sensitive_words:
            if word.lower() in message.lower():
                for m in re.finditer(re.escape(word), message, re.IGNORECASE):
                    ...
                    if val not in ignore_tokens:
                        findings["commits"].append(
                            {"type": f"Sensitive Word: {word}", "commit": commit_hash[:8], "match": val}
                        )
        ...
        for m in candidates:
            ...
            if entropy >= entropy_threshold:
                findings["commits"].append(
                    {"type": "ENTROPY", "commit": commit_hash[:8], "token": token, "entropy": round(entropy, 2)}
                )
...
def scan_reflog(
    ...
    for file_path, line_no, content in added_lines:
        ...
        for m in candidates:
            ...
            if entropy >= entropy_threshold:
                findings["entropy"].append(
                    {"file": reflog_path, "line": line_no, "token": token, "entropy": round(entropy, 2)}
                )
...
def scan_diff(
    ...
    for file_path, line_no, content in added_lines:
        ...
        for m in candidates:
            ...
            if entropy >= entropy_threshold:
                findings["entropy"].append(
                    {"file": file_path, "line": line_no, "token": token, "entropy": round(entropy, 2)}
                )
    return findings
