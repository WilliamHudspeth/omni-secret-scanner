...
def scan_current_tree_parallel(
    ...
    *file_jobs* is a list of job tuples (as assembled by scan_current_tree).
    ...
    if n_files == 0:
        return {
            "suspicious_files": [], "current_secrets": [],
            "nlp_pii": [], "injections": [], "taint": [], "stego": [],
        }
    ...
    try:
        ...
        with ctx(max_workers=n_workers) as executor:
            futures = {
                executor.submit(_worker_scan_chunk, chunk): chunk
                for chunk in chunks
            }
            completed = 0
        ...
        if not quiet:
            print(f"ProcessPool failed ({e}), falling back to threaded scan...",
                  file=sys.stderr)
