...
_MODE_PREF_PATH = Path.home() / ".rgt-scan.toml"


def _load_last_mode() -> str | None:
    """Return 'quick' or 'full' from ~/.rgt-scan.toml, or None."""
    if not _MODE_PREF_PATH.exists():
        return None
    try:
        raw = _MODE_PREF_PATH.read_text(encoding="utf-8")
        for line in raw.splitlines():
            if line.startswith("last_mode"):
                val = line.split("=", 1)[-1].strip().strip('"').strip("'")
                return val if val in ("quick", "full") else None
    except Exception:
        pass
    return None
...
def _save_last_mode(mode: str) -> None:
    try:
        _MODE_PREF_PATH.write_text(f'last_mode = "{mode}"\n', encoding="utf-8")
    except Exception:
        pass


def mode_picker_menu(args) -> str:
    """Show the Quick / Full scan mode picker.  Returns 'quick' or 'full'."""
    options = [
        ("Quick Scan", "stdlib only · secrets · PII · entropy · git history · results in <5 s"),
        (
            "Full Scan",
            "all detectors · NLP · AST · Semgrep · external tools · requires [all] extras",
        ),
    ]
    last = _load_last_mode()
    selected = 0 if last != "full" else 1
    ...
    while True:
        ...
        print(f"┌{border}┐")
        pad = " " * ((width - 22) // 2)
        print(f"│{pad}  RGT Codebase Scanner  {pad}│")
        print(f"│{' ' * width}│")
        print(f"│  Select scan mode:{' ' * (width - 19)}│")
        print(f"│{' ' * width}│")
        for i, (name, desc) in enumerate(options):
            cursor = ">" if i == selected else " "
            label = f"  {cursor} [{i + 1}] {name:<12}  {desc}"
            label = label[: width - 1].ljust(width)
            print(f"│{label}│")
        print(f"│{' ' * width}│")
        hint = "  Arrow keys to move  •  Enter to select  •  q to quit"
        print(f"│{hint:<{width}}│")
        print(f"└{border}┘")
    ...
    mode = "quick" if selected == 0 else "full"
...
def run_tui(args) -> None:
    ...
    # Show mode picker when invoked interactively (no subcommand implies interactive)
    if getattr(args, "interactive", False):
        mode = mode_picker_menu(args)
        if mode == "full":
            # Enable all optional detectors for full scan
            args.nlp_pii = True
            args.presidio = True
            args.semgrep = True
    state = {
    ...
    while True:
        menu_picker("RGT CODEBASE SCANNER", options, selected)
        key = get_key()
        if key == "up":
            selected = (selected - 1) % len(options)
        elif key == "down":
