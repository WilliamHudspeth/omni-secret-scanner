...
"""Command-line entry point for rgt-codebase-scanner."""
...
def run_self_test(quiet: bool = False) -> bool:
    ...
    print(f"\nrgt-codebase-scanner v{__version__} — Self-Test Results")
    print("=" * 55)
...
def print_tool_schema() -> None:
    ...
    schema = {
        "name": "scan_secrets",
        "description": (
            f"rgt-codebase-scanner v{__version__}: Scan a code snippet or text for hardcoded secrets, "
...
def autofix_gitignore(files_to_add: list[str], dry_run: bool = False) -> int:
    ...
    with open(gitignore_path, "a", encoding="utf-8") as fp:
        fp.write("\n# rgt-codebase-scanner autofix additions\n")
        for entry in to_add:
            fp.write(f"{entry}\n")
...
def _install_hook(strict: bool = False) -> None:
    ...
    hook_content = (
        "#!/usr/bin/env bash\n"
        "echo 'Running rgt-codebase-scanner...'\n"
        f"rgt-scan{extra_args}\n"
        "if [ $? -ne 0 ]; then\n"
...
def main(argv: list[str] | None = None) -> int:  # noqa: C901
    ...
    if args.watch:
        from rgt_codebase_scanner.detectors.file_tree import _scan_single_file
    ...
    if getattr(args, "decay", False):
        from rgt_codebase_scanner.utils.decay import apply_decay_to_findings
    ...
    if getattr(args, "gitleaks", False):
        from rgt_codebase_scanner.detectors.external import run_gitleaks

        ext_findings.extend(run_gitleaks(repo_dir, quiet=args.quiet))
    if getattr(args, "trivy", False):
        from rgt_codebase_scanner.detectors.external import run_trivy
